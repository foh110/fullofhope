...
def main():
    ...
    st.write(f"1. optimal_discount类型：{type(model_info['optimal_discount'])}，值：{model_info['optimal_discount']}")
    st.write(f"2. fixed_rsp类型：{type(fixed_rsp)}，值：{fixed_rsp}")
    st.write(f"3. recommended_price类型：{type(recommended_price)}，值：{recommended_price}")
    # 输入罐单价（原逻辑不变）
